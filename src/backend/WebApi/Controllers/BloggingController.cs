...
public class BloggingController(ILogger<BloggingController> logger, IBloggingRepository bloggingRepository) : ControllerBase
{
    ...
    public async Task<Ok<List<BlogDto>>> GetBlogs(CancellationToken cancellationToken)
    {
        logger.LogInformation("GetBlogs was called");
        return TypedResults.Ok((await bloggingRepository.ListBlogsAsync(cancellationToken)).ToList());
    }
    ...
    public async Task<Results<Ok<BlogDto>, NotFound>> GetBlog(int id, CancellationToken cancellationToken)
    {
    ...
    public async Task<Results<Ok<BlogDto>, NotFound>> PostBlog(BlogDto blog, CancellationToken cancellationToken)
    {
        ...
        return TypedResults.Ok(blogEntry);
    }
    ...
    public async Task<Ok<List<PostDto>>> GetPosts(int blogId, CancellationToken cancellationToken)
    {
        logger.LogInformation("GetPosts was called with id {BlogId}", blogId);
        return TypedResults.Ok((await bloggingRepository.ListPostsAsync(blogId, cancellationToken)).ToList());
    }
    ...
    public async Task<Results<Ok<PostDto>, NotFound>> GetPost(int id, CancellationToken cancellationToken)
    {
    ...
    public async Task<Results<Ok<PostDto>, NotFound>> PostPost(PostDto post, CancellationToken cancellationToken)
    {
        ...
        return TypedResults.Ok(postEntry);
    }
}